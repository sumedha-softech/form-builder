...
const FormBuilderCanvas = ({
  ...
  const [draggedField, setDraggedField] = useState({ sectionId: null, fieldIndex: null });
  ...
  const handleFieldDragOver = (e, sectionId, index) => {
    e.preventDefault();
    if (draggedField.sectionId !== sectionId) return;
    if (index === draggedField.fieldIndex) return;

    const updatedSections = [...formFields];
    const section = updatedSections.find((s) => s.id === sectionId);

    const draggedItem = section.fields.splice(draggedField.fieldIndex, 1)[0];
    section.fields.splice(index, 0, draggedItem);

    setDraggedField({ sectionId, fieldIndex: index });
    setFormFields(updatedSections);
  };
                        ...
                        {section.fields
                          ...
                          .map((field, index) => (
                            <div
                              ...
                              draggable
                              onDragStart={() => setDraggedField({ sectionId: section.id, fieldIndex: index })}
                              onDragOver={(e) => handleFieldDragOver(e, section.id, index)}
                              onDragEnd={() => setDraggedField({ sectionId: null, fieldIndex: null })}
                            >
