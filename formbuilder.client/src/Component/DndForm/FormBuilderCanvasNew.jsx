...
function SortableSection({ section, children, onDeleteField, selectedSection, onSelectSection }) {
    ...
    return (
        <div ref={setNodeRef} style={style} className={`${section.width} mb-2 p-0`}>
            <div
                className={`form-section card cursor-pointer h-100 ${
            ...
            >
                <div className="card-header">
                    <div className="d-flex justify-content-between align-items-start">
                        ...
                        <button
                            type="button"
                            className="btn btn-link text-muted p-1 cursor-grab"
                            {...attributes}
                            {...listeners}
                        >
...
function SortableField({ field, selectedField, onSelectField, onDeleteField, sectionField, children }) {
    ...
    return (
        <div ref={setNodeRef} className={field.width + " " + field.alignment + " " + field.align + " mb-3"}>
            <div
                className={`form-field position-relative p-3 border border-2 rounded cursor-pointer ${
            ...
            >
                {isSelected && (
                    <div className="action-buttons d-flex position-absolute top-0 end-0">
                        ...
                        <button
                            type="button"
                            className="btn btn-sm btn-outline-info py-1 pb-2 px-2 lh-1"
                            {...attributes}
                            {...listeners}
                        >
                            <GripVertical size={16} />
                                ...
                                .map((section) => (
                                    ...
                                    >
                                        <div
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                const isInsideField = section.fields.some(
                                                    (f) =>
                                                        fieldRefs.current[f.id] &&
                                                        fieldRefs.current[f.id].contains(e.target)
                                                );
                                                if (!isInsideField) {
                                                    onSelectSection(section);
                                                }
