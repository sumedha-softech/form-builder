const PropertiesPanel = ({ selectedField, onUpdateField, selectedSection, onUpdateSection, propertyPanelRef }) => {
  ...
  return (
    <div className="properties-panel p-3 overflow-auto" ref={propertyPanelRef}>
      ...
      {selectedField ? (
        <>
          ...
          <div className="mb-3">
            ...
            <select className="form-select form-select-sm" value={selectedField.type} onChange={(e) => handleFieldUpdate({ type: e.target.value })}>
      ...
      {selectedField && ["checkbox", "radio", "select"].includes(selectedField.type) && (
        <>
          <hr />
          <h6 className="fw-medium mb-3 small">{selectedField.type.charAt(0).toUpperCase() + selectedField.type.slice(1)} Options</h6>
          ...
          <button
            ...
            onClick={() => handleFieldUpdate({ options: [...(selectedField.options || []), `Option ${selectedField.options.length + 1}`] })}
          >
            Add Option
          </button>
          ...
          {selectedField.type === "checkbox" && (
            <div className="form-check mt-3">
              <input
                ...
                className="form-check-input"
                id="single-select"
                checked={selectedField.singleSelect}
                onChange={(e) => handleFieldUpdate({ singleSelect: e.target.checked })}
              />
              <label className="form-check-label small" htmlFor="single-select">
                Single Select (acts like radio buttons)
              </label>
            </div>
          )}
        </>
